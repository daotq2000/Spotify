...
import { paginationAlbum,deleteAlbum } from '../../redux/albumReducer'
...
const headCells = [
    ...
    { id: 'albumName', numeric: false, disablePadding: false, label: 'Tên Album' },
    { id: 'totalListen', numeric: true, disablePadding: false, label: 'Lượt nghe' },
];
...
const EnhancedTableToolbar = (props) => {
    ...
    const handleOK =(option) =>{
        if(option == 'ok'){
            dispatch(deleteAlbum(listRemove))
            reset = true;
        }
        setOpen(false);
    }
        ...
        >
            {numSelected > 0 ? (
                ...
                <>
                    ...
                    <Button onClick={() => history.push(`/admin/create-album`)} style={{ fontSize: 'small', width: '200px' }} variant="contained" color="primary" >
                        <QueueIcon /> &ensp; Tạo Album
                    </Button>
                </>
            )}
...
export default function EnhancedTable() {
    ...
    useEffect(() => {
        initialState();
    }, [state.albums, state.totalElements, state.currentPage, state.totalPages])
                            ...
                            {(rows != undefined ? rows : []).map((row, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell align="left"><Link to={`/admin/update-album/${row.id}`}>{row.albumName}</Link></TableCell>
                                        <TableCell align="right">{row.totalListen}</TableCell>
                                    </TableRow>
                                );
                            })}
                            {emptyRows > 0 && (
